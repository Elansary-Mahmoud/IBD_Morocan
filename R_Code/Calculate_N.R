...
print(Disease_name)
nodes <- detectCores()
registerDoMC(nodes/2)
print(nodes)

Disease_code = as.numeric(switch(Disease_name, "IBD"="1", "CD"="2", "UC"="3") )

f = file.path("/scratch/Mahmoud/IBD_Morocan/Regions_N",paste(Disease_name,"_HD_",1:163,".assoc.logistic.mperm",sep=""))
assoc.linear.mperm <- lapply(f[file.exists(f)], read.table,header=T,stringsAsFactors=F)

f = file.path("/scratch/Mahmoud/IBD_Morocan/Regions_N",paste(Disease_name,"_HD_",1:163,".assoc.logistic",sep=""))
...
write.table(results,file=paste("/scratch/Mahmoud/IBD_Morocan/",Disease_name,"_pos_regions_pred_N.txt"),quote=F,row.names=F)
...
sumulation_sum = NULL
restart_times = 0
plink = "/scratch/Mahmoud/plink-1.07-x86_64/plink"
Genofile = "/scratch/Mahmoud/IBD_Morocan/AllImmunochipsMarocains-29102012-build37"
phenofile = "/scratch/Mahmoud/IBD_Morocan/Fichier_Phenotypes_final_bis.txt"
outfile = "/scratch/Mahmoud/IBD_Morocan/simulations_regions_v2/"
################### user Defined Function  ###################
##############################################################
		...
		if(!is.na(results[i,2])){
			...
			while(restart == 1){
				...
				if(length(summary(as.factor(random_region[,1]))) == 1){
					...
					assoc.linear = NULL
					assoc.linear.mperm = NULL	
					f = file.path("/scratch/Mahmoud/IBD_Morocan/simulations_regions_v2",paste("simulation_s_",s_no,"_",i,".assoc.logistic",sep=""))
					assoc.linear <<- lapply(f[file.exists(f)], read.table,header=T,stringsAsFactors=F)
		
					f = file.path("/scratch/Mahmoud/IBD_Morocan/simulations_regions_v2",paste("simulation_s_",s_no,"_",i,".assoc.logistic.mperm",sep=""))
					assoc.linear.mperm <<- lapply(f[file.exists(f)], read.table,header=T,stringsAsFactors=F)
					pred_n = predict_n(1)
					if(!is.na(pred_n)){
					 	if(pred_n >= (results[i,2] - 2) & pred_n <= (results[i,2] + 2) ) {
							...
							write.table(output,file=paste("/scratch/Mahmoud/IBD_Morocan/simulations_regions_v2/sa_",s_no,"_",i,".txt",sep=""),quote=F)	 	
					 	} else{	
					 		restart = 1	 	
...
for(s in start:(end - 1))
{
	...
	x <- foreach(i=1:163, .combine='c') %dopar% sample_Region(i)
